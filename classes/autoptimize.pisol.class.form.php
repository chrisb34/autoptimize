...
if(!class_exists('autoptimize_pisol_class_form')):
class autoptimize_pisol_class_form{
    ...
    */
    function multiselect_box(){
        ...
        $desc = ((isset($this->setting['desc'])) ? '<br><small>'.$this->setting['desc'].'</small>' : "");
        $field = '<select class="form-control '.$this->pro.'" name="'.$this->setting['field'].'[]" id="'.$this->setting['field'].'" multiple'. '>';
            foreach($this->setting['value'] as $key => $val){
